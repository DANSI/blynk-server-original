    ...
    protected void messageReceived(ChannelHandlerContext ctx, User user, Message msg) {
        switch (msg.command) {
            case HARDWARE_COMMAND :
                hardware.messageReceived(ctx, user, (HardwareMessage) msg);
                break;
            case PING :
                PingLogic.messageReceived(ctx, msg.id);
                break;
            case EMAIL :
                email.messageReceived(ctx, user, (MailMessage) msg);
                break;
            case BRIDGE :
                bridge.messageReceived(ctx, user, (BridgeMessage) msg);
                break;
            case PUSH_NOTIFICATION :
                push.messageReceived(ctx, user, (PushMessage) msg);
                break;
            case TWEET :
                tweet.messageReceived(ctx, user, (TweetMessage) msg);
                break;
        }
    }
